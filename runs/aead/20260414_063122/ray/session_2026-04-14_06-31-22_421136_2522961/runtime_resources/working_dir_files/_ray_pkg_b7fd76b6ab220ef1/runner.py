...
def _validate_state_no_aliasing(interp):
    ...
    # Build map: mem_map -> [(ptr_name, base_addr, size)]
    ranges_by_map = defaultdict(list)
    for var_name, arr_infos in interp.arr_inputs.items():
        ...
        for arr_info in arr_infos:
            if '.shadow' in arr_info.mem_map:
                continue
            base_addr = env.get_concrete_value(var_name)
            if base_addr is None:
                continue
            if isinstance(base_addr, int):
                size = arr_info.elem_size * arr_info.num_elements
                ranges_by_map[arr_info.mem_map].append(
                    (var_name, base_addr, base_addr + size)
                )

    for mem_map, ranges in ranges_by_map.items():
...
def run_native(program, program_inputs, test_name, input_name, extra_data=None, log_read=True, pickled=False, compiled=None, no_trace=False):
    ...
    If no_trace=True, skips trace building (only returns explored blocks)."""
    try:
        import swoosh_interp
    except ImportError:
        raise ImportError(
...
def write_trace_binary_v2(path, raw_trace):
    """Write v2 binary trace with raw int encoding.

    raw_trace has keys: pc_values, block_values, op_values, pc_registry, block_registry, total.
    Value sections map string keys to sets of raw ints (not pickled).
    """
    import zstandard as zstd

    VALUE_CATS = {0: 'pc_values', 1: 'block_values', 2: 'op_values'}
    total = raw_trace.get('total', 0)
    cctx = zstd.ZstdCompressor(level=3, threads=-1)

    with open(path, 'wb') as fh:
        with cctx.stream_writer(fh) as writer:
            writer.write(b"SWTR")
            writer.write(struct.pack('<BQ', 2, total))  # version=2

            for cat_id in range(5):
                if cat_id in VALUE_CATS:
                    section = raw_trace.get(VALUE_CATS[cat_id], {})
                    writer.write(struct.pack('<BI', cat_id, len(section)))
                    for key_str, members in section.items():
                        key_bytes = key_str.encode() if isinstance(key_str, str) else key_str
                        writer.write(struct.pack('<H', len(key_bytes)))
                        writer.write(key_bytes)
                        writer.write(struct.pack('<BI', 1, len(members)))  # enc_flag=1
                        buf = bytearray(len(members) * 8)
                        for i, v in enumerate(members):
                            if isinstance(v, bytes):
                                v = int.from_bytes(v, 'little', signed=False)
                            struct.pack_into('<Q', buf, i * 8, v & ((1 << 64) - 1))
                        writer.write(buf)

                elif cat_id == 3:  # pc_registry
                    section = raw_trace.get('pc_registry', {})
                    writer.write(struct.pack('<BI', cat_id, len(section)))
                    for key_str, pcs in section.items():
                        key_bytes = key_str.encode() if isinstance(key_str, str) else key_str
                        writer.write(struct.pack('<H', len(key_bytes)))
                        writer.write(key_bytes)
                        writer.write(struct.pack('<BI', 2, len(pcs)))  # enc_flag=2
                        buf = bytearray(len(pcs) * 4)
                        for i, pc in enumerate(pcs):
                            struct.pack_into('<I', buf, i * 4, pc)
                        writer.write(buf)

                elif cat_id == 4:  # block_registry
                    section = raw_trace.get('block_registry', {})
                    writer.write(struct.pack('<BI', cat_id, len(section)))
                    for key_str, blocks in section.items():
                        key_bytes = key_str.encode() if isinstance(key_str, str) else key_str
                        writer.write(struct.pack('<H', len(key_bytes)))
                        writer.write(key_bytes)
                        members = [pickle.dumps(b) for b in blocks]
                        writer.write(struct.pack('<BI', 0, len(members)))  # enc_flag=0
                        for m in members:
                            writer.write(struct.pack('<H', len(m)))
                            writer.write(m)
...
def process_single_input(input_file, test_name, test_path, engine='python',
    ...
    try:
        ...
        if engine == 'python':
            ...
            explored, raw_trace = run_native(
                program, program_inputs, test_name, input_name,
                extra_data=program_inputs.extra_data,
                log_read=not no_read_trace,
                pickled=False,  # Raw ints for v2 binary format
                compiled=compiled,
            )
            # Write v2 binary format directly (skip .compact.pkl)
            write_trace_binary_v2(bin_path, raw_trace)
        elif engine == 'both':
