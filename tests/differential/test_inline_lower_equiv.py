...
stay behaviorally equivalent to Boogie `/printInlined` (the oracle).
...
def test_inline_lower_behavioral_cases(behavioral, tmp_path):
    """Each hand-written case: Rust inline_lower must match Boogie + pass asserts."""
...
def test_inline_lower_fuzz(behavioral, seed, tmp_path):
    ...
    match Boogie + satisfy the composed-result asserts."""
...
def test_inline_lower_benchmark_differential(name, ct):
    """Real-SMACK differential: globals preserved + final memory matches Boogie."""
    pytest.importorskip("swoosh_interp")
    import os
    ...
    pre, boogie_inlined = build_programs(bpl, ct)
    assert structural_compare(name, pre, boogie_inlined), f"{name}: structural"
    assert behavioral_compare(name, pre, boogie_inlined), f"{name}: behavioral"
